...
# Filter rows for NBA MVP award and winner
nba_mvp_winners = merged_player_df[(merged_player_df['award_award'] == 'nba mvp') & (merged_player_df['winner_award'] == True)]
nba_mvp_winners['target'] = 1  # Create a new column 'target' and set it to 1 for MVP winners
Y = merged_player_df.merge(nba_mvp_winners[['player_adv', 'season_adv', 'target']], on=['player_adv', 'season_adv'], how='left')['target'].fillna(0)

# Begin with Logistic Regression. Split into train, test, and validate sets
X_train, X_test, y_train, y_test = train_test_split(X_encoded, Y.astype(int), test_size=0.2, random_state=42)
X_train2, X_val, y_train2, y_val = train_test_split(X_train, y_train, test_size=0.2, random_state=42)

# Fit regression model and predict on the validation set
